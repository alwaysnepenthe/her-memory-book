...
(() => {
  ...
  function pageContent(record,n,side){
    ...
    if(side==='left'){
      const date=el('time','journal-date',record.date.replaceAll('-',' / '));date.dateTime=record.date;const age=ageAt(record.date);if(age)date.append(el('span','record-age',' · '+age));content.append(date,el('h2','journal-entry-title',record.title));
      ...
      const foot=el('div','entry-actions'),edit=el('button','text-button','✎ 编辑本页');edit.onclick=()=>{if(turning||opening)return;pauseVideos();config.onEdit?.(record,isDemo());};const read=el('button','text-button','展开阅读 ↗');read.onclick=()=>{pauseVideos();if(isDemo())config.onRead?.(record);else config.onOpen?.(record.id);};foot.append(edit,read);content.append(foot);
    }else{
  ...
  function webDraw(){const web=$('#growthWeb');web.replaceChildren();const list=rows.length?rows:[null];list.forEach((r,n)=>{const card=el('article','growth-web-card');if(r)card.dataset.recordId=r.id;const story=pageContent(r,n,'left');story.className='timeline-story';const notes=story.querySelector('.paper-notes');if(notes){const paragraphs=notes.textContent.split(/\n\s*\n/);notes.replaceChildren(...paragraphs.map(text=>el('p','timeline-paragraph',text)));}story.querySelectorAll('.botanical-corner,.paper-heading,.paper-number').forEach(e=>e.remove());const date=el('header','timeline-date');const time=story.querySelector('.journal-date');if(time)date.append(time);const body=el('div','timeline-body');body.append(story);if(r?.media.length){const media=pageContent(r,n,'right');media.className='timeline-media';media.querySelectorAll('.botanical-corner,.paper-heading,.paper-number').forEach(e=>e.remove());body.append(media);}card.append(date,body);web.append(card);});}
  function syncLayout(){pauseVideos();$('.motion-choice').hidden=layout==='web';$('#showBook').setAttribute('aria-pressed',String(layout==='book'));$('#showWeb').setAttribute('aria-pressed',String(layout==='web'));$('#growthWeb').hidden=layout!=='web';$('#bookCoverStage').hidden=layout!=='book'||opened;$('#bookReader').hidden=layout!=='book'||!opened;if(layout==='web')webDraw();else $('#growthWeb').replaceChildren();}
  ...
  function ageAt(date){const birth=config.pet.birthday;if(!birth||date<birth)return '';const [y,m,d]=date.split('-').map(Number),[by,bm,bd]=birth.split('-').map(Number);const months=(y-by)*12+m-bm-(d<bd?1:0);if(months<1){const days=Math.floor((new Date(date+'T12:00:00Z')-new Date(birth+'T12:00:00Z'))/86400000);return days===0?'出生当天':days+' 天大';}return months<12?months+' 个月大':Math.floor(months/12)+' 岁'+(months%12?' '+months%12+' 个月':'');}
  function directory(){const box=$('#growthDirectory');box.replaceChildren();let month='';if(!rows.length){box.append(el('p','','当前筛选下还没有记录。'));return;}rows.forEach((r,n)=>{if(month!==r.date.slice(0,7)){month=r.date.slice(0,7);box.append(el('h3','',month.replace('-',' 年 ')+' 月'));}const b=el('button','directory-entry',r.date.slice(8)+' 日 · '+r.title+(ageAt(r.date)?' · '+ageAt(r.date):''));b.type='button';b.onclick=()=>{index=n;if(layout==='book')setOpen(true);else{syncLayout();$('#growthWeb').children[n]?.scrollIntoView?.({behavior:reduced()?'instant':'smooth',block:'start'});}};box.append(b);});}
  function cover(){directory();$('#sampleContext').hidden=!isDemo();const p=config.pet;$('#bookCoverPhoto').src=p.avatar||window.HER_SEED[p.portraitIndex??3]?.data||placeholder();$('#bookCoverPhoto').alt=(p.name||'宠物')+'的成长手账照片';$('#bookPetName').textContent=(p.name||'小伙伴')+'的小日子';$('#coverCount').textContent='共 '+rows.length+' 页小日常 · 轻触封面，慢慢翻开';$('#bookMode').value=isDemo()?'demo':'real';syncJournalMenus();}
