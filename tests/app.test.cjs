...
test('every select uses shared styling and profile restoration stays synchronized',async()=>{const cleanup=[];try{const factory=new IDBFactory(),a=await launch(factory,cleanup);const selects=[...a.w.document.querySelectorAll('select')];assert.equal(selects.length,5);for(const select of selects){assert.equal(select.hidden,true);assert.ok(select.closest('.journal-select'));assert.ok(a.$('#'+select.id+'Trigger'));}a.$('[data-view="growth"]').click();a.$('#showWeb').click();a.$('#growthOrderTrigger').click();a.$('#growthOrderOptions [data-value="desc"]').click();assert.equal(a.$('#growthOrder').value,'desc');assert.ok(a.$('#growthWeb article').textContent.includes('让这个午后慢一点'));a.$('[data-view="profile"]').click();a.$('#profileSexTrigger').click();a.$('#profileSexOptions [data-value="female"]').click();assert.ok(a.$('#profileSaveStatus').textContent.includes('未保存'));a.submit('#profileForm');await until(()=>!a.$('#profileSaveStatus').textContent.includes('未保存'));a.$('#profileSexTrigger').click();a.$('#profileSexOptions [data-value="male"]').click();a.$('#resetProfile').click();assert.equal(a.$('#profileSex').value,'female');assert.equal(a.$('#profileSexTrigger').textContent,'女孩');const b=await launch(factory,cleanup);assert.equal(b.$('#profileSexTrigger').textContent,'女孩');assert.equal(a.errors.length,0);assert.equal(b.errors.length,0);}finally{cleanup.forEach(f=>f());}});
